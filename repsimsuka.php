...
<html>
<head>
	<title>Laporan Nominatif Simpanan Manasuka</title>
	<style>
		@media print{
			input.noPrint{
				display: none;
...
<table border="1" width="100%" style="border-collapse: collapse;">
	...
	<caption><h2>NOMINATIF SIMPANAN MANASUKA</h2></caption>
	...
	<tbody>
		<?php
			$no=1;
			$sql=$koneksi->query("select * from member where simsuka>0 order by username");
			$total = 0;
			while($data=$sql->fetch_assoc()){
				$total = $total+$data['simsuka'];
			?>	
			<tr>
				<td><?php echo $no++;?></td>
				<td><?php echo $data['username'];?></td>
				<td><?php echo $data['nama'];?></td>	
				<td align="right"><?php echo number_format($data['simsuka'],0,',','.');?></td>
