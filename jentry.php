...
<div style="border:0; padding:10px; width:924px; height:auto;">
	<div>
		<h1>
			<center><label class="label label-success">Jurnal Entry</label></center>
		</h1><br><br>
		...
		if (isset($_POST['buku'])) {
			...
			if ($vcket == '') {
				...
				if ($vdtgl == $date) {
					...
					$ii = $cnt1->fetch_assoc();
					$numrow = mysqli_num_rows($cnt1); // ✅ BENAR
					if ($numrow == 0) {
						$vnnotrans = 1;
					} else {
						$maxtr = $ii['maxtr'];
				...
				if ($totdebet1 == $totkredit1) {
					...
					while ($duta = $sql1->fetch_assoc()) {
						...
						// Tambahkan 2 parameter tambahan
						$cno_bukti = ''; // atau $_POST['cno_bukti'];
						$cproject = '';  // atau $_POST['cproject'];

						jurnal($koneksi, $vdtgl, $vcket, $vcnokira, $vcdebkred, $vnidramount, $user, $vnnotrans, $cno_bukti, $cproject);
					}
