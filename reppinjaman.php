...
<html>
<head>
	<title>Laporan Nominatif Pinjaman</title>
	<style>
		@media print{
			input.noPrint{
				display: none;
...
<table border="1" width="100%" style="border-collapse: collapse;">
	...
	<caption><h2>NOMINATIF PINJAMAN</h2></caption>
	...
	<tbody>
		<?php
			$no=1;
			$sql=$koneksi->query("select * from member where pinjaman>0 order by username");
			$total = 0;
			while($data=$sql->fetch_assoc()){
				$total = $total+$data['pinjaman'];
			?>	
			<tr>
				<td><?php echo $no++;?></td>
				<td><?php echo $data['username'];?></td>
				<td><?php echo $data['nama'];?></td>	
				<td><?php echo $data['dduedate'];?></td>
				<td align="right"><?php echo number_format($data['pinjaman'],0,',','.');?></td>
			...
			?>
			<tr>
				<td colspan="4" align="right"><b>TOTAL OUTSTANDING</b></td>
